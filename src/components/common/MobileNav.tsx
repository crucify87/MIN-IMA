...
const MobileNav = ({ view, setView }: MobileNavProps) => {
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 lg:hidden bg-white/95 backdrop-blur-xl border-t border-outline-variant/30 flex items-center justify-around py-4 px-6 z-[60] safe-area-inset-bottom shadow-2xl">
      {menuItems.map((item) => (
        <button
          key={item.id}
        ...
        >
          ...
          {(view === item.id || (view === 'detail' && item.id === 'inventory')) && <div className="w-1 h-1 rounded-full bg-primary absolute -bottom-1" />}
        </button>
      ))}
    </nav>
  );
